...
(defun boolean-eval (exp)
  ...
  (let* (
    ...
    (format t "~%~%~%bool_operator :  ~a" bool_operator)
    (format t "~%operands:  ~a" operands)
    (format t "~%operand list length:  ~a" operands_len)
    (format t "~%left_operand:  ~a" left_operand)
    (format t "~%right operand:  ~a" (car(cdr operands)))
...
;---or------ "or" expression check
	  ( 
	   (equal bool_operator 'or) ; bool_operator == and
...
;---ERROR------ "xor" expression check (using "boolean-xor" method)
	  ( 
	   (equal bool_operator 'xor) ; bool_operator == and
...
;---implies------ "implies" expression check (use "boolean-implies" method)
	  ( 
	   (equal bool_operator 'implies) ; bool_operator == and
...
;---iff------ "implies" expression check (use "boolean-implies" method)
	  ( 
	   (equal bool_operator 'iff) ; bool_operator == and
